...
function parseIf(tokens) {
  ...
  if (tokens[0] == IF_LEFT) {
    condition = 'isPathLeft';
  }
  else if (tokens[0] == IF_RIGHT) {
    condition = 'isPathRight';
  }
  else if (tokens[0] == IF_FORWARD) {
    condition = 'isPathForward';
  }

  tokens.splice(0, 1);

  var innerSequence = parseSequence(tokens);
  var blockType = (tokens[0] == ELSE) ? 'maze_ifElse' : 'maze_if';
  var innerElseEnd = parseElseEnd(tokens);

  if (innerSequence == 'error' || innerElseEnd == 'error') return 'error';
  return '<block type="' + blockType + '"><field name="DIR">' + condition + '</field><statement name="DO">' + innerSequence + '</statement>' + innerElseEnd + '</block>';
}
...
function parseElseEnd(tokens) {
  ...
  var inner = parseSequence(tokens);
  if (inner == 'error' || tokens[0] != END_IF) return 'error';
